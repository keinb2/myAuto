...
skip_time = get_start_time(coin)
while True:
    try:
        now = datetime.datetime.now()+datetime.timedelta(hours=9)
        start_time = get_start_time(coin)
        if skip_time == start_time or skip_time < start_time:
            skip_time = start_time
            end_time = start_time + datetime.timedelta(minutes=10)
            if start_time < now and now < (end_time - datetime.timedelta(seconds=7)):
                point = check_point(coin,gap_ratio,position)
                #print(point)
                if point == -1 :
                    krw = get_balance("KRW")
                    if krw > krwUnit:
                        buy_result = upbit.buy_market_order(coin,krwUnit)
                        #post_message(myToken,myChannel,"Target buy : " +str(buy_result))
                    else :
                        print("Try to buy but no KRW")
                    skip_time = end_time
                elif point == 1 :
                    t_coin = get_balance(targetCoin)
                    if t_coin > tcoinUnit:
                        sell_result = upbit.sell_market_order(coin,tcoinUnit)
                        #post_message(myToken,myChannel,"Target sell : " +str(sell_result))
                    else :
                        print("Try to sell but no targets")
                    skip_time = end_time
